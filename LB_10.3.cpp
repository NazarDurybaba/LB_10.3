...
void saveToFile(const vector<Employee>& employees, const string& filename) {
    ...
    for (const auto& employee : employees) {
        file << employee.surname << " " << employee.initials << " " << employee.position << " "
            << employee.hire_year << " " << employee.salary << "\n";
    }
    file.close();
}
...
void loadFromFile(vector<Employee>& employees, const string& filename) {
    ...
    Employee employee;
    while (file >> employee.surname >> employee.initials >> employee.position >> employee.hire_year >> employee.salary) {
